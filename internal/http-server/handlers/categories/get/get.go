package get

import (
	"errors"
	"log/slog"
	"net/http"

	resp "seams-backend/internal/lib/api/response"
	"seams-backend/internal/lib/logger/sl"
	"seams-backend/internal/models"
	"seams-backend/internal/storage"

	"github.com/go-chi/chi/v5"
	"github.com/go-chi/chi/v5/middleware"
	"github.com/go-chi/render"
	"github.com/google/uuid"
)

type Response struct {
	resp.Response
	Category *models.Category `json:"category,omitempty"`
}

type CategoryGetter interface {
	GetCategoryByID(id uuid.UUID) (*models.Category, error)
}

func New(log *slog.Logger, getter CategoryGetter) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		const op = "handlers.category.get.New"

		log := log.With(
			slog.String("op", op),
			slog.String("request-id", middleware.GetReqID(r.Context())),
		)

		idStr := chi.URLParam(r, "id")
		if idStr == "" {
			log.Error("missing category id in request")

			render.Status(r, http.StatusBadRequest)
			render.JSON(w, r, resp.Error("missing category id"))

			return
		}

		id, err := uuid.Parse(idStr)
		if err != nil {
			log.Error("invalid category id format", sl.Err(err))

			render.Status(r, http.StatusBadRequest)
			render.JSON(w, r, resp.Error("invalid category id format"))

			return
		}

		cat, err := getter.GetCategoryByID(id)
		if err != nil {
			if errors.Is(err, storage.ErrNotFound) {
				log.Error("category not found", sl.Err(err))

				render.Status(r, http.StatusNotFound)
				render.JSON(w, r, resp.Error("category not found"))
				return
			}

			log.Error("failed to get category by id", sl.Err(err))

			render.Status(r, http.StatusInternalServerError)
			render.JSON(w, r, resp.Error("internal server error"))

			return
		}

		render.Status(r, http.StatusOK)
		responseOK(w, r, cat)
	}
}

func responseOK(w http.ResponseWriter, r *http.Request, category *models.Category) {
	render.JSON(w, r, Response{
		Response: resp.OK(),
		Category: category,
	})
}
