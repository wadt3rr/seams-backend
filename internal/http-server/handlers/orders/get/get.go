...
type Response struct {
	resp.Response
	Order *models.OrderDetailsDTO `json:"order"`
}

type OrderGetter interface {
	GetOrderDetails(ctx context.Context, id uuid.UUID) (*models.OrderDetailsDTO, error)
}
...
func New(log *slog.Logger, getter OrderGetter) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		order, err := getter.GetOrderDetails(r.Context(), id)
		if err != nil {
			if errors.Is(err, storage.ErrNotFound) {
				log.Error("order not found", sl.Err(err))
			...
			log.Error("failed to get order by id", sl.Err(err))
...
func responseOK(w http.ResponseWriter, r *http.Request, order *models.OrderDetailsDTO) {
